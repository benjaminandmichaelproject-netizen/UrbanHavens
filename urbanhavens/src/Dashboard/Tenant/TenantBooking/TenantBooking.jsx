...
import { api, clearTenantBooking } from "../../Owner/UploadDetails/api/api";
      ...
      {images.length > 1 && (
        <>
          ...
          <button
            ...
            onClick={() => setIdx((p) => (p === images.length - 1 ? 0 : p + 1))}
          >
            <FaChevronRight />
          </button>
          ...
          <div className="tb-img-dots">
            {images.map((_, i) => (
              <span
                key={i}
                className={`tb-img-dot ${i === idx ? "active" : ""}`}
                onClick={() => setIdx(i)}
              />
            ))}
          </div>
...
const TenantBooking = () => {
  ...
  const getImages = (b) => {
    if (Array.isArray(b.property_images) && b.property_images.length > 0) {
      return b.property_images;
    }
  ...
  const handleClearBooking = async (bookingId) => {
    try {
      ...
      console.error("Clear tenant booking error:", err);
      alert(err?.detail || "Failed to clear booking.");
    } finally {
      setClearingId(null);
    }
  };
  ...
  const renderClearButton = (booking) => {
    if (booking.status !== "converted") return null;
    ...
    return (
      ...
      >
        <FaTrashAlt />
        {clearingId === booking.id ? "Clearing..." : "Clear"}
      </button>
    );
  };
                    ...
                    <tr key={b.id}>
                      ...
                      <td>
                        <div className="tb-prop-cell">
                          ...
                          <span>
                            <FaMapMarkerAlt />
                            {[b.property_city, b.property_region].filter(Boolean).join(", ")}
                          </span>
                      ...
                      <td>
                        <div className="tb-owner-cell">
                          ...
                          <a href={`tel:${b.owner_phone}`} className="tb-inline-contact tb-phone">
                            <FaPhone /> {b.owner_phone || "—"}
                          </a>
                          <a href={`mailto:${b.owner_email}`} className="tb-inline-contact tb-email">
                            <FaEnvelope /> {b.owner_email || "—"}
                          </a>
                        </div>
                      </td>
                      ...
                      <td>
                        <div className="tb-prop-cell">
                          <span><FaCalendarAlt /> {formatDate(b.preferred_date)}</span>
                          <span>{formatTime(b.preferred_time)}</span>
                        </div>
                      </td>

                      <td>
                        <span className={`tb-badge ${meta.cls}`}>
                      ...
                      <td>
                        <button className="tb-view-btn" onClick={() => setSelected(b)}>
                          View
                        </button>
                      </td>

                      <td>{renderClearButton(b)}</td>
                    </tr>
                  );
                })
              ) : (
                <tr>
                  <td colSpan="9" className="tb-empty-row">
                    <FaHome /> No bookings found.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="tb-modal-backdrop" onClick={() => setSelected(null)}>
          <div className="tb-modal" onClick={(e) => e.stopPropagation()}>
            <div className="tb-modal-header">
              ...
              <button className="tb-close-btn" onClick={() => setSelected(null)}>
                <FaTimes />
              </button>
            </div>
            ...
            <div className="tb-modal-body">
              ...
              <div className="tb-modal-status-row">
                ...
                <span className="tb-modal-date">
                  <FaCalendarAlt /> {formatDate(selected.created_at)}
                </span>
              </div>
              ...
              <div className="tb-modal-section">
                ...
                <div className="tb-modal-contact-btns">
                  <a href={`tel:${selected.owner_phone}`} className="tb-contact-btn tb-phone">
                    <FaPhone /> Call Owner
                  </a>
                  <a href={`mailto:${selected.owner_email}`} className="tb-contact-btn tb-email">
              ...
              <div className="tb-modal-section">
                <h4><FaUser /> Your Booking Request</h4>
                <div className="tb-modal-grid">
                  <div><span>Preferred Date</span><strong>{formatDate(selected.preferred_date)}</strong></div>
                  <div><span>Preferred Time</span><strong>{formatTime(selected.preferred_time)}</strong></div>
              ...
              {selected.status === "converted" && (
                <div className="tb-modal-section">
                  <button
                    ...
                    onClick={() => handleClearBooking(selected.id)}
                    disabled={clearingId === selected.id}
                  >
                    <FaTrashAlt />
                    {clearingId === selected.id ? "Clearing..." : "Clear Booking"}
                  </button>
                </div>
              )}
            </div>
